...
class Datanode:
    ...
    def set_alive(self):
        ...
        if not self.is_alive:
            logger.info("Datanode: {uid} -- Offline".format(uid = self.uid))
        else:
            logger.info("Datanode: {uid} -- Online".format(uid = self.uid))
    ...
    def print_datanode(self):
        logger.info("id: {uid},location {location}, is online? {is_alive}".format(uid = self.uid, location = self.location, is_alive = self.is_alive))
